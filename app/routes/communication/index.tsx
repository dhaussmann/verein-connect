...
import { NavLink, Form, useActionData, useLoaderData } from "react-router";
...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  const messages = await getMessagesDataUseCase(env, user.orgId);
  return { messages };
}
...
export default function CommunicationMessagesRoute() {
  ...
  return (
    <div>
      ...
      <Group gap="sm" mb="md" wrap="wrap"><Select value="all" w={144} placeholder="Kanal" data={[{ value: "all", label: "Alle Kanäle" }]} /><Select value="all" w={144} placeholder="Status" data={[{ value: "all", label: "Alle" }]} /><div style={{ marginLeft: "auto" }}><Button component={NavLink} to="/communication/email">Neue Nachricht</Button></div></Group>
