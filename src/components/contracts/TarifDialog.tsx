  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{isEdit ? 'Tarif bearbeiten' : 'Neuer Tarif'}</DialogTitle>
        </DialogHeader>
        ...
        <Tabs defaultValue="basisdaten" className="mt-2">
          ...
          <TabsContent value="basisdaten" className="space-y-6 mt-4">
              ...
              <div>
                ...
                >
                  <div className="flex items-center gap-1.5">
                    <RadioGroupItem value="true" id="t-active-yes" />
                    <Label htmlFor="t-active-yes" className="font-normal">Ja</Label>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <RadioGroupItem value="false" id="t-active-no" />
                    <Label htmlFor="t-active-no" className="font-normal">Nein</Label>
                  </div>
                </RadioGroup>
            ...
            {membershipTypes.length > 0 && (
              <div>
                <Label className="text-xs uppercase text-muted-foreground">Erlaubte Mitgliedschaftsarten</Label>
                <div className="border rounded-md p-3 mt-1 space-y-2 max-h-[150px] overflow-y-auto">
                  {membershipTypes.map(mt => (
                    <div key={mt.id} className="flex items-center gap-2">
                      <Checkbox
                        id={`mt-${mt.id}`}
                        checked={form.allowed_membership_type_ids.includes(mt.id)}
                        onCheckedChange={() => toggleMembershipType(mt.id)}
                      />
                      <Label htmlFor={`mt-${mt.id}`} className="font-normal text-sm">{mt.name}</Label>
                    </div>
                  ))}
                </div>
